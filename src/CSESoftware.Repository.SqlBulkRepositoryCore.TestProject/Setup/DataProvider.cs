...
internal static class DataProvider
{
    internal static List<FamilyTree> GetSimpleTrees(int numberOfTrees = 10, string gender = "Tomato",
        bool isAlive = true) =>
        Enumerable.Range(0, numberOfTrees).Select(x => GetTree(DateTime.UtcNow - TimeSpan.FromDays(x), gender, isAlive))
            .ToList();
    ...
    private static FamilyHome GetHome(string name, string address) => new()
    {
        Id = Guid.NewGuid(),
        IsActive = true,
        CreatedDate = DateTime.Now,
