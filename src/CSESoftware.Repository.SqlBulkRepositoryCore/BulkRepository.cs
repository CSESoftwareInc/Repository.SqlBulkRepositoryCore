...
public class BulkRepository<TContext>(TContext context, int batchSize = 50000) : Repository<TContext>(context), IRepositoryBulk
    where TContext : DbContext
{
    ...
    {
        if (selectByValues.Count == 0) return [];

        const OperationType type = OperationType.Select;
        var batchedEntities = RepositoryHelper.SplitLargeVolumesOfEntities(selectByValues.ToList(), batchSize);
        ...
        where TEntity : IEntity
    {
        ...
        var batchedEntities = RepositoryHelper.SplitLargeVolumesOfEntities(entities.ToList(), batchSize);
        var conn = (SqlConnection)Context.Database.GetDbConnection();
    ...
    public async Task BulkUpdateAsync<TEntity, TObject>(TEntity entityToUpdate,
        ...
        where TEntity : IEntity
    {
        if (updateByValues.Count == 0) return;

        const OperationType type = OperationType.Update;
        var batchedEntities = RepositoryHelper.SplitLargeVolumesOfEntities(updateByValues.ToList(), batchSize);
        var conn = (SqlConnection)Context.Database.GetDbConnection();
        var mustOpen = conn.State != ConnectionState.Open;
    ...
    public async Task BulkDeleteAsync<TEntity, TObject>(TEntity entityToDelete,
        ...
        where TEntity : IEntity
    {
        if (deleteByValues.Count == 0) return;

        const OperationType type = OperationType.Delete;
        var batchedEntities = RepositoryHelper.SplitLargeVolumesOfEntities(deleteByValues.ToList(), batchSize);
        var conn = (SqlConnection)Context.Database.GetDbConnection();
        var mustOpen = conn.State != ConnectionState.Open;
