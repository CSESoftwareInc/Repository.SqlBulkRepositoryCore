...
internal static class RepositoryHelper
{
    ...
    /// <param name="typeToEvaluate">A sample of the anonymous object to be inserted into the temp table</param>
    /// <returns>A string for use in parameterized queries</returns>
        ...
        where TEntity : IEntity
    {
        ...
        var returnList = operationObjectProperties.Where(x =>
                primaryKeyProperties.ContainsKey(x) || primaryKeyProperties.ContainsValue(x))
        ...
        where TEntity : IEntity
    {
        ...
        var returnList = updateProperties.Where(x =>
                !primaryKeyProperties.ContainsKey(x) && !primaryKeyProperties.ContainsValue(x) &&
                (entityProperties.ContainsKey(x) || entityProperties.ContainsValue(x)))
            .Select(prop => ConstructMatchString(prop, entityProperties)).ToList();
